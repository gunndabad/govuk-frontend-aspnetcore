...
public class EncodedAttributesDictionary : IReadOnlyDictionary<string, string>
{
    ...
    private EncodedAttributesDictionary(TagHelperAttributeList attributes)
    {
    ...
    /// <summary>
    /// Creates a new <see cref="EncodedAttributesDictionary"/> from the an existing <see cref="ReadOnlyTagHelperAttributeList"/>.
    /// </summary>
    ...
    public static EncodedAttributesDictionary FromTagHelperAttributes(ReadOnlyTagHelperAttributeList tagHelperAttributes) =>
        new EncodedAttributesDictionary(new TagHelperAttributeList(tagHelperAttributes));
    ...
    /// <returns><see langword="true"/> if the attribute is successfully found and removed.</returns>
    /// <exception cref="ArgumentNullException" />
    public bool Remove(string name, [NotNullWhen(true)] out string? encodedValue)
    {
        ArgumentNullException.ThrowIfNull(name);

        if (_attributes.TryGetAttribute(name, out var attribute))
        {
            encodedValue = GetEncodedAttributeValue(attribute);
    ...
    private static TagHelperAttribute CreateAttribute(string name, string value, bool encodeValue) =>
    ...
    private static string GetEncodedAttributeValue(TagHelperAttribute attribute) =>
        attribute.ValueStyle == HtmlAttributeValueStyle.Minimized ?
            ...
            {
                IHtmlString htmlString => htmlString.ToHtmlString(),
                string str => str,

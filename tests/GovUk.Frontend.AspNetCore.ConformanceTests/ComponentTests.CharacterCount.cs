    ...
    public void CharacterCount(ComponentTestCaseData<OptionsJson.CharacterCount> data) =>
        CheckComponentHtmlMatchesExpectedHtml(
            ...
            (generator, options) =>
            {
                ...
                var countMessageAttributes = new AttributeDictionary().MergeAttribute(
                    "class",
                    options.CountMessage?.Classes
                );
                ...
                return generator
                    .GenerateCharacterCount(
                        ...
                        options.CharactersUnderLimitText is not null
                            ? (options.CharactersUnderLimitText.Other, options.CharactersUnderLimitText.One)
                            : null,
                        options.CharactersAtLimitText,
                        options.CharactersOverLimitText is not null
                            ? (options.CharactersOverLimitText.Other, options.CharactersOverLimitText.One)
                            : null,
                        options.WordsUnderLimitText is not null
                            ? (options.WordsUnderLimitText.Other, options.WordsUnderLimitText.One)
                            : null,
                        options.WordsAtLimitText,
                        options.WordsOverLimitText is not null
                            ? (options.WordsOverLimitText.Other, options.WordsOverLimitText.One)
                            : null
                    )
                    .ToHtmlString();
            }

    ...
    public void Tabs(ComponentTestCaseData<OptionsJson.Tabs> data) =>
        CheckComponentHtmlMatchesExpectedHtml(
            ...
            (generator, options) =>
            {
                ...
                var items = (
                    options.Items?.Select(i => new TabsItem()
                    {
                        Id = i.Id,
                        Label = i.Label,
                        ...
                        PanelContent =
                            !string.IsNullOrEmpty(i.Panel.Text)
                                ? new HtmlString(
                                    "<p class=\"govuk-body\">" + HtmlEncoder.Default.Encode(i.Panel.Text) + "</p>"
                                )
                ...
                return generator.GenerateTabs(options.Id, options.IdPrefix, title, attributes, items).ToHtmlString();
            }
        );
}
